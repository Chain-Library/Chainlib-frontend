...
const EarningsSummary = ({ earningsSumaryDetails }: EarningsSummaryProps) => {
  return (
    <div className="w-full mx-auto flex items-start flex-col gap-6 bg-[var(--color-background)] min-h-[181px] shadow-[0px_4px_12px_#1212120A] rounded-lg justify-center p-4">
      <div className="w-full flex flex-row gap-6 items-center flex-wrap justify-between">
        {earningsSumaryDetails.map((item, index) => (
          <div
            key={index}
            className="w-[257px] py-3 px-6 flex flex-col gap-2 items-start justify-center rounded-lg border-[1px]"
      ...
      <Button className="text-sm font-normal text-[var(--color-background)] px-8 py-5 bg-[linear-gradient(180deg,_#096CFF_40.7%,_#054199_180.61%)]">
        Request Payment
      </Button>
    </div>
  )
